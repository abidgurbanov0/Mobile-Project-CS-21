...
import com.example.muricerr.adapter.PlaylistTrackRecyclerViewAdapter
...
class PlaylistTrackFragment : Fragment() {
    ...
    private lateinit var trackAdapter: PlaylistTrackRecyclerViewAdapter
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_playlist_track, container, false)
        ...
        removeButton = view.findViewById(R.id.unsave_button)

        val trackIds = arguments?.getLongArray("TRACK_ID")?.toList()
        trackIds?.let { viewModel.getTracksByIds(it) }

        trackAdapter = PlaylistTrackRecyclerViewAdapter(
            tracks = emptyList(),
            onItemClick = { track -> displayPlayer(track) },
            onRemoveTrackClick = { track -> removeTrackFromPlaylist(track) }
        )
    ...
    private fun removeTrackFromPlaylist(track: Track) {
        ...
        val updatedTracks = viewModel.tracks.value?.filter { it.id != track.id } ?: emptyList()
